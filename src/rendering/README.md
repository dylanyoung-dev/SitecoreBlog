...
test/preview at [www.sitecoreblogtest.localhost](https://www.sitecoreblogtest.localhost).
Be sure to watch the `rendering` container logs for compilation errors. To debug, you
can attach to the `SitecoreBlogTest.exe` process inside the `rendering` container.
